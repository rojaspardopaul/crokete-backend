...
const customerInvoiceEmailBody = (option) => {
  ...
  const ok = '#10B981';
  const ac = '#0EA5E9';
  const sc = { Pending:'#F59E0B', Processing:'#3B82F6', Delivered:'#10B981', Cancel:'#EF4444' };
  const statusColor = sc[option.status] || bc;
  const statusMap = { Pending:'Pendiente', Processing:'En Proceso', Delivered:'Entregado', Cancel:'Cancelado' };
  ...
  const cartRows = option.cart.map(function(item) {
    ...
    return '<tr style="border-bottom:1px solid ' + br + ';">'
      ...
      + '<td style="padding:14px 16px;font-family:Helvetica Neue,Helvetica,Arial,sans-serif;font-size:14px;color:' + tp + ';text-align:right;">' + cur + Number(item.price).toFixed(2) + '</td>'
      + '<td style="padding:14px 16px;font-family:Helvetica Neue,Helvetica,Arial,sans-serif;font-size:14px;color:' + tp + ';text-align:right;font-weight:600;">' + cur + (item.price * item.quantity).toFixed(2) + '</td>'
      + '</tr>';
  }).join('');
